...
// great_gross	1728
// Task 1
// Store the unit of measurement in your program
...
// Opens in a modal
// Task 3
// Add an item to the customer bill
...
func RemoveItem(bill, units map[string]int, item, unit string) bool {
	...
	if unitsVal, ok := units[unit]; ok {
		...
		if _, ok := bill[item]; ok {
			...
			if bill[item]-unitsVal < 0 {

				return false

			} else if bill[item]-unitsVal == 0 {
...
func GetItem(bill map[string]int, item string) (amount int, ok bool) {
    amount, ok = bill[item]
    return
}